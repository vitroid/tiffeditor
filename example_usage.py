...
def basic_usage_example():
    ...
    try:
        ...
        with TiffEditor(
            ...
            print(
                f"青いボックス: {read_blue.shape}, 平均色値: {np.mean(read_blue, axis=(0,1))}"
            )
            print(
                f"赤いボックス: {read_red.shape}, 平均色値: {np.mean(read_red, axis=(0,1))}"
            )
...
def scalable_editor_example():
    ...
    try:
        ...
        with ScalableTiffEditor(
            ...
            print(
                f"緑のボックス: {read_green.shape}, 平均色値: {np.mean(read_green, axis=(0,1))}"
            )
            print(
                f"青のボックス: {read_blue.shape}, 平均色値: {np.mean(read_blue, axis=(0,1))}"
            )
            ...
            virtual_size = (virtual_shape[0] * virtual_shape[1] * virtual_shape[2]) / (
                1024 * 1024
            )
